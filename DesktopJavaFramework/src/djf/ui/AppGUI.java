...
public class AppGUI {
    ...
    /**
     * Accessor method for getting the undo toolbar, within which all
     * undo/redo controls are placed.
     * 
    ...
    /**
     * This method is used to activate/deactivate toolbar buttons after each
     * work edit such that they can be usable or not usable in order to
     * enforce the rules of foolproof design.
     *
    ...
    private void initTopToolbar(AppTemplate app) {
        ...
        if (isTrue(HAS_TOP_TOOLBAR)) {
            ...
            if (isTrue(HAS_FILE_TOOLBAR)) {
                initFileToolbar();            
            }
            if (isTrue(HAS_CUT_TOOLBAR)) {
                initCutToolbar();
            }
            if (isTrue(HAS_UNDO_TOOLBAR)) {
                initUndoToolbar();
            }
            if (isTrue(HAS_SETTINGS_TOOLBAR)) {
                initSettingsToolbar();
            }
        }
    }
    ...
    /**
     * Note that this is the default style class for the buttons in the top
     * toolbars and that style characteristics for this type of component
